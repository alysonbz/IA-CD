...
def preparar_base(df, limite_missing=0.50):
    ...
    A imputação é feita no Pipeline para evitar data leakage.
    """
    df = df.copy()
    ...
    colunas_para_remover = [
        col for col in colunas_identificadoras if col in df.columns]
    ...
    print(
        f"Colunas removidas por excesso de valores ausentes (>= 50%): {len(colunas_muito_missing)}")
...
def plotar_reta_regressao_simples(df, atributo):
    ...
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / "reta_regressao_simples.png", dpi=150)
    plt.close()
    ...
    print("\nEquação da reta:")
    print(
        f"{TARGET} = {modelo.intercept_:.4f} + ({modelo.coef_[0]:.4f} * {atributo})")

    return modelo


def plotar_retas_todas_features_melhor_grid(df_transf, selected_features, df_grid, tabela_corr):
    """
    ...
    """
    nome_melhor = df_grid["R2_teste"].idxmax()
    info = df_grid.loc[nome_melhor]
    alpha = info["melhor_alpha"]

    if nome_melhor == "Ridge":
        modelo_base = Ridge(alpha=float(alpha))
    elif nome_melhor == "Lasso":
        modelo_base = Lasso(alpha=float(alpha), max_iter=20000)
    else:
    ...
    pipe = construir_pipeline(modelo_base)
    ...
    alpha_str = f"α={alpha}" if alpha != "-" else "sem regularização"
    fig.suptitle(
        f"Efeito parcial por feature — {nome_melhor} ({alpha_str})  |  R²={info['R2_teste']:.4f}",
        fontsize=11, y=1.01
    )
    plt.tight_layout()
...
def plotar_reta_melhor_grid(df_transf, atributo, df_grid):
    """
...
def grid_search(X_train, X_test, y_train, y_test, kfold):
    ...
    # Regressão Linear não tem alpha — avalia direto sem GridSearch
    pipe_lr = construir_pipeline(LinearRegression())
    pipe_lr.fit(X_train, y_train)
    metricas_lr = calcular_metricas(y_test, pipe_lr.predict(X_test))
    from sklearn.model_selection import cross_val_score
    r2_cv_lr = cross_val_score(
        pipe_lr, X_train, y_train, cv=kfold, scoring="r2").mean()
    resultados_grid["Regressão Linear"] = pipe_lr
    linhas.append({
        "modelo": "Regressão Linear",
        "melhor_alpha": "-",
        "melhor_R2_CV": r2_cv_lr,
        "MAE_teste": metricas_lr["MAE"],
        "MSE_teste": metricas_lr["MSE"],
        "RMSE_teste": metricas_lr["RMSE"],
        "R2_teste": metricas_lr["R2"],
    })
...
def main():
    ...
    _, df_grid = grid_search(X_train, X_test, y_train, y_test, kfold)

    plotar_reta_melhor_grid(df_transf, atributo_simples, df_grid)
    plotar_retas_todas_features_melhor_grid(
        df_transf, selected_features, df_grid, tabela_corr)
