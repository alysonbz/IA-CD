...
# --------------------
# 1. Melhor configuração encontrada (StandardScaler + k=17)
...
# 2. Aplicar Cross-validation (5 folds)
scores = cross_val_score(knn, X_scaled, y, cv=5, scoring='accuracy')
print(f"\nMédia da Acurácia (5 folds): {scores.mean():.4f}")
print(f"Desvio Padrão da Acurácia: {scores.std():.4f}")


# 3. Previsões para Matriz de Confusão e Relatório de Classificação
